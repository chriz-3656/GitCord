...
export class SecurityService {
    static SECRET_PATTERNS = [
        {
            ...
            remediation: 'Never commit .env files. Add .env to .gitignore and rotate any exposed secrets.',
        },
        {
            name: 'AWS Access Key',
            regex: /AKIA[0-9A-Z]{16}/,
            severity: SecuritySeverity.CRITICAL,
            remediation: 'Revoke this key immediately in AWS console. This key may have been compromised.',
        },
        {
        ...
        {
            name: 'Slack Token',
            regex: /(xoxb|xoxp|xoxa|xox0|xox1|xox2|xox3|xox4|xox5|xox6|xox7|xox8|xox9|xoxr)-[a-zA-Z0-9-]{150,}/,
            severity: SecuritySeverity.CRITICAL,
    ...
    static detectSecurityIssues(content, commits, changedFiles) {
        const issues = [];
            ...
            for (const file of changedFiles) {
                if (this.DANGEROUS_FILES.some((df) => file.endsWith(df))) {
                    issues.push({
                        ...
                        remediation: 'Verify this change is intentional and review all modifications to this file.',
                        details: file,
                    });
                }
            }
            ...
            for (const commit of commits) {
                ...
                if ((commit.added?.length || 0) < 5 && (commit.removed?.length || 0) > 20) {
                    issues.push({
                        ...
                        remediation: 'Verify deletions are intentional and no important files were removed by accident.',
        ...
        if (eventType === 'push') {
            if (event.forced) {
                return {
                    ...
                    remediation: 'Review recent commits to ensure no important changes were overwritten. Consider protecting this branch.',
                };
            }
            if (event.deleted) {
                return {
