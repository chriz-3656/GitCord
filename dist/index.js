...
app.post('/webhooks/github', async (req, res) => {
    ...
    try {
        ...
        if (securityAlerts.length > 0 || suspiciousActivity) {
            const securityEmbed = SecurityService.createSecurityAlertEmbed(repoFullName, securityAlerts.length > 0 ? securityAlerts : [suspiciousActivity], req.body.sender?.login || 'Unknown');
