...
app.post('/webhooks/github', async (req, res) => {
    ...
    try {
        ...
        const securityIssues = SecurityService.detectSecurityIssues(rawPayloadString, commits, changedFiles);
        ...
        if (allIssues.length > 0) {
            ...
            try {
                ...
                const securityAlerts = guild.channels.cache.find((ch) => ch.isTextBased() && ch.name === 'security-alerts');
                if (securityAlerts) {
                    securityChannel = securityAlerts;
                }
            }
            catch (e) {
        ...
        switch (event) {
            ...
            case 'release': {
                ...
                if (releaseData) {
                    ...
                    try {
                        const guild = channel.guild;
                        const releaseType = ReleaseService['determineReleaseType'](releaseData.version, releaseData.isPrerelease);
                        ...
                        if (releaseChannel) {
                            ...
                            if (ReleaseService['shouldPinRelease'](releaseType)) {
                                try {
                                    ...
                                    logger.warn(`Could not pin release message: ${pinError.message}`);
                                }
                            }
                            ...
                            if (ReleaseService['shouldCreateThread'](releaseType)) {
                                try {
                                    ...
                                    logger.warn(`Could not create thread: ${threadError.message}`);
                                }
                            }
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        const contributor = await ContributorService.getOrCreateByDiscordId(user.id, user.username, user.displayAvatarURL());
        try {
            if (action === 'like') {
                ...
                await interaction.reply({
                    content: result.action === 'added' ? '🔔 You are now following this project!' : '🔕 Unfollowed.',
                    ephemeral: true,
                });
            }
