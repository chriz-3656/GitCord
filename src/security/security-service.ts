...
export class SecurityService {
  ...
  static createSecurityAlertEmbed(repoFullName: string, alerts: string[], pusher: string) {
    return new EmbedBuilder()
      .setColor(0xff0000)
      .setTitle('⚠ SECURITY ALERT')
      .setDescription(`Potential security risk detected in a recent push to **${repoFullName}**`)
      .addFields(
        { name: 'Detected Issues', value: alerts.map((a) => `• ${a}`).join('\n') },
  ...
  static isSuspiciousActivity(event: any, eventType: string): string | null {
    if (eventType === 'push') {
      if (event.forced) return 'Force push detected';
      if (event.deleted) return 'Branch deletion detected';
    }
    return null;
  }
}
