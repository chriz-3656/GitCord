...
export class ContributorService {
  static async updateStats(data: {
    ...
    const update: any = {};
    if (data.type === 'commit') update.commits = { increment: 1 };
    if (data.type === 'pr') update.prs = { increment: 1 };
    if (data.type === 'issue') update.issues = { increment: 1 };
    if (data.type === 'star') update.stars = { increment: 1 };
