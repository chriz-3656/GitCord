...
import { SecurityIssue, SecuritySeverity } from '../../security/security-service.js';
...
export interface ReleaseCardInput {
  ...
  releasedAt: Date;
  body: string;
  assets: Array<{ name: string; url: string; size: number; downloadCount: number }>;
  releaseUrl: string;
  isDraft?: boolean;
}
...
const severityColor = (severity: SecuritySeverity): number => {
  switch (severity) {
...
const createText = (content: string) => new TextDisplayBuilder().setContent(content);
...
export class CardFactory {
  ...
  static createShowcaseCard(input: ShowcaseCardInput): ComponentsV2Reply {
    ...
    if (input.bannerUrl) {
      components.push(
        new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(input.bannerUrl)),
      );
    }
      ...
      new ContainerBuilder()
        ...
        .addSectionComponents(
          new SectionBuilder().addTextDisplayComponents(
            createText(`**Pushed by:** ${input.pusher}`),
            createText(commitLines),
          ),
        ),
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder()
  ...
  static createPRCard(input: PRCardInput): ComponentsV2Reply {
    ...
    const section = new SectionBuilder()
      .addTextDisplayComponents(
        createText(`🔀 **PR ${input.action.charAt(0).toUpperCase() + input.action.slice(1)}: #${input.number} ${input.title}**`),
        ...textParts,
      )
  ...
  static createIssueEventCard(input: WebIssueCardInput): ComponentsV2Reply {
    ...
    const section = new SectionBuilder()
      .addTextDisplayComponents(
        createText(`❗ **Issue ${input.action.charAt(0).toUpperCase() + input.action.slice(1)}: ${input.title}**`),
        ...textParts,
      )
      .setThumbnailAccessory(new ThumbnailBuilder({ media: { url: input.avatarUrl } }));
  ...
  static createStarCard(input: StarCardInput): ComponentsV2Reply {
    const section = new SectionBuilder()
      .addTextDisplayComponents(
        createText(`⭐ **New Star on ${input.repoName}**`),
        createText(`**Starred by:** ${input.starrer}`),
        createText('A community member just starred this repository.'),
      )
      .setThumbnailAccessory(new ThumbnailBuilder({ media: { url: input.avatarUrl } }));
  ...
  static createLeaderboardCard(input: LeaderboardCardInput): ComponentsV2Reply {
    ...
    return this.createReply([
      new ContainerBuilder()
        ...
        .addTextDisplayComponents(
          createText(`🏆 **${input.title}**`),
          createText(input.subtitle),
          createText(lines.join('\n')),
        )
        .addSeparatorComponents(
          new SeparatorBuilder().setDivider(true).setSpacing(SeparatorSpacingSize.Small),
        ),
  ...
  static createProfileCard(input: ProfileCardInput): ComponentsV2Reply {
    ...
    const section = new SectionBuilder()
      .addTextDisplayComponents(
        createText(`👤 **${input.username}**`),
        createText(`**Tier:** ${input.tier}`),
        createText(`**Reputation:** ${input.reputation} points`),
        createText(
          [
  ...
  static createReleaseCard(input: ReleaseCardInput): ComponentsV2Reply {
            ...
            .map(
              (asset) =>
                `• [${asset.name}](${asset.url}) — ${
                  asset.size > 0 ? `${Math.round(asset.size / 1024)} KB` : 'unknown size'
                } | ⬇️ ${asset.downloadCount}`,
            )
            .join('\n')
        : 'No assets available.';
    ...
    const section = new SectionBuilder()
      .addTextDisplayComponents(
        createText(`🚀 **${input.title}**`),
        createText(
          `**Version:** \`${input.version}\` • **Tag:** \`${input.tagName}\` • **Type:** ${input.type}`,
        ),
        createText(`**Released:** <t:${Math.floor(input.releasedAt.getTime() / 1000)}:R>`),
        createText(input.body || 'No changelog provided.'),
        createText(`**Downloads**\n${assetLines}`),
      )
  ...
  static createSecurityCard(input: SecurityCardInput): ComponentsV2Reply {
    if (input.issues.length === 0) {
      const section = new SectionBuilder().addTextDisplayComponents(
        createText('✅ **Security Scan Passed**'),
        createText(`No security issues detected in **${input.repoFullName}**`),
        createText(`**Pushed by:** ${input.pusher}`),
      );
    ...
    const section = new SectionBuilder().addTextDisplayComponents(
      createText(`⚠️ **Security Alert - ${highest.severity}**`),
      createText(`**Repository:** ${input.repoFullName}`),
      createText(`**Pushed by:** ${input.pusher}`),
      createText(`**Findings:** ${input.issues.length}`),
      createText(`**Summary**\n${summary}`),
      createText(`**Issues**\n${issueText}`),
    );
